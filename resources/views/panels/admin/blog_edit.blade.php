...
@section('content')
    ...
    {!! Form::open(['url' => route('admin.update-blog'), 'class' => 'form-signin', 'data-parsley-validate','enctype'=>'multipart/form-data' ] ) !!}

    <label for="question">Page Title</label>
    <input name="page_title" class="blog_title_lavel" value="<?php echo($blogs->page_title)?>">&nbsp&nbsp&nbsp&nbsp
    <label for="question">Blog Title</label>
    <input name="title" class="blog_title_lavel" value="<?php echo($blogs->title)?>">
    <h3>Summary</h3>
    <textarea class="form-control my-editor" name="summary" ><?php echo($blogs->summary)?></textarea>
    <h3>Description</h3>
    <textarea class="form-control my-editor" name="blog" ><?php echo($blogs->contents)?></textarea>
    ...
    <br/>
    <input type="hidden" name="blog_id" value="<?php echo($blogs->id)?>">
    <button style="margin-top: 20px" type="submit" class="btn btn-default">Update</button>
    {{--</form>--}}
    {!! Form::close() !!}
